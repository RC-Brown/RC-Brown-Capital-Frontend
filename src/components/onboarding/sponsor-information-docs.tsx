...
export interface SponsorInformationDocsRef {
  validate: () => boolean;
}

export const SponsorInformationDocs = forwardRef<SponsorInformationDocsRef, SponsorInformationDocsProps>(
  ({ value = { additionalDocs: [] }, onChange, error }, ref) => {
    ...
    // Validate that track record is uploaded
    const validateSponsorInformationDocs = () => {
      if (!trackRecordFile) {
    ...
    useEffect(() => {
      if (error) {
        validateSponsorInformationDocs();
      }
    ...
    useImperativeHandle(ref, () => ({
      validate: () => validateSponsorInformationDocs(),
    }));
...
SponsorInformationDocs.displayName = "SponsorInformationDocs";
