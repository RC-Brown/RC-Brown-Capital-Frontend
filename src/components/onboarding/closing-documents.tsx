...
import { ClosingDocument } from "@/src/types/onboarding";
...
export const ClosingDocuments = forwardRef<ClosingDocumentsRef, ClosingDocumentsProps>(({ onChange, error }, ref) => {
  ...
  useEffect(() => {
    const storedDocs = formData.closing_documents as ClosingDocument[] | undefined;
  ...
  const handleTypeChange = (documentType: string) => {
    ...
    if (docType && uploadedFile) {
      const docs = [{ document_name: docType.value, files: [uploadedFile] }];
      onChange(docs);
      updateFormData({ closing_documents: docs });
    } else if (docType) {
      const docs = [{ document_name: docType.value, files: [] }];
      onChange(docs);
      updateFormData({ closing_documents: docs });
    }
  };
  ...
  const handleFileSelect = (selectedFiles: FileList | null) => {
    ...
    if (acceptedFileTypes.includes(file.type) || file.name.match(/\.(pdf|doc|docx|txt|xls|xlsx|jpg|jpeg|png)$/i)) {
      ...
      if (docType) {
        const docs = [{ document_name: docType.value, files: [file] }];
        onChange(docs);
        updateFormData({ closing_documents: docs });
      }
    }
