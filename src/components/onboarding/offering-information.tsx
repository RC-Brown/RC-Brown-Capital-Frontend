...
export interface OfferingInformationRef {
  validate: () => boolean;
}
...
export const OfferingInformation = forwardRef<OfferingInformationRef, OfferingInformationProps>(
  ({  onChange, error }, ref) => {
    ...
    // Validate that both information type and file are selected
    const validateOfferingInformation = () => {
    ...
    useImperativeHandle(ref, () => ({
      validate: () => validateOfferingInformation(),
    }));
          ...
          <DialogContent className='max-w-lg rounded-2xl bg-white p-6 pt-10'>
            <DialogHeader>
              ...
              <DialogDescription className='text-center text-sm text-[#858585]'>
                Please upload the offering information for{" "}
                {informationTypes.find((t) => t.value === selectedType)?.label}
              </DialogDescription>
            </DialogHeader>
...
OfferingInformation.displayName = "OfferingInformation";
